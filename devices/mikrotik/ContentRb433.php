...
class ContentRb433 extends ContentInstaller
{
    ...
    public function install()
    {
        ...
        if ($this->recordExists('{{%device}}', ['vendor'=> 'Mikrotik', 'model' => 'RB433'])) {
            throw new \Exception('Device Mikrotik RB433 already exists');
        }

        /** Insert new Device Mikrotik RB433  */
        ...
        $device = $this->getEntryIdentifier('{{%device}}', ['vendor'=> 'Mikrotik', 'model' => 'RB433'], 'id');

        /** Add device attributes */
        $this->command->batchInsert('{{%device_attributes}}', ['device_id', 'sysobject_id', 'hw', 'sys_description'], [
            [$device, '1.3.6.1.4.1.14988.1', null, 'RouterOS RB433']
        ])->execute();
