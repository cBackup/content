...
class ContentAsabackupSsh extends ContentInstaller
{
    ...
    public function install()
    {
        ...
        if ($this->recordExists('{{%worker}}', ['name' => 'cisco_asa_backup'])) {
            throw new \Exception('Worker cisco_asa_backup already exists');
        }

        $this->command->insert('{{%worker}}', [
            'name'      => 'cisco_asa_backup',
            'task_name' => 'backup',
            'get'       => 'ssh'
        ])->execute();

        /** Get newly inserted worker id */
        $worker = $this->getEntryIdentifier('{{%worker}}', ['name'=> 'cisco_asa_backup'], 'id');

        /** Add worker jobs */
        $this->command->batchInsert('{{%job}}', ['name', 'worker_id', 'sequence_id', 'command_value', 'timeout', 'table_field'], [
            ['terminal_length', $worker, 1, 'terminal pager 0', null, null],
            ['show_ru', $worker, 2, 'show ru', 60000, 'config'],
            ['terminal_length_back', $worker, 3, 'terminal pager 24', null, null],
